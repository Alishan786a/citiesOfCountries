...
const ReactAlert=lazy(()=>import('../Alert'));


let AdminHomeComp=(props)=>{
...
// this will help to chech which advertisment is selected
let formRef=useRef()
...
if (result2?.data) {
    ...
    {myAlert.display && <ReactAlert color={`${myAlert.color}`} sms={`${myAlert.sms}`} />}
        ...
        <Container className="adminAdvertismentList">

            {props.advertismentList.length !==0? props.advertismentList.map((e)=>{
return (
          <Row className="customeClr rounded my-2 align-items-center py-1" key={e._id}>
            ...
            <Col xs md={2} >
<Row className="">
    <Col><AiFillDelete className="text-danger fs-4" onClick={()=>{props.deleteAdvertis(e._id,alertFunc)}}/></Col>
